...
function renderArticle(article) {
    ...
    const categoryHtml = `<span class="badge-tag badge-${(article.category || 'announcement').toLowerCase()}">${(article.category || 'Announcement').toUpperCase()}</span>`;

    const hasImage = article.featuredImage !== 'none';
    const imageHtml = hasImage ? `<img src="${article.featuredImage || article.image || 'supreme_logo1.webp'}" class="full-article-image" alt="Featured">` : '';
