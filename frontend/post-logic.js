...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (newThreadFromPostForm) {
    ...
    // Modal Live Preview listener
    const modalContent = document.getElementById('post-thread-content');
    const modalPreview = document.getElementById('thread-preview-area');
    if (modalContent && modalPreview) {
        modalContent.addEventListener('input', () => {
            const val = modalContent.value;
            if (val.trim()) {
                modalPreview.innerHTML = window.formatRichText(val);
            } else {
                modalPreview.innerHTML = '<span style="color: var(--text-secondary); font-style: italic;">Preview will appear here as you type...</span>';
            }
        });
    }
  }

  // Reply Live Preview listener
  ...
  if (replyContent && replyPreview) {
      replyContent.addEventListener('input', () => {
          ...
          if (val.trim()) {
              replyPreview.innerHTML = window.formatRichText(val);
          } else {
              replyPreview.innerHTML = '<span style="color: var(--text-secondary); font-style: italic;">Preview will appear here as you type...</span>';
          }
      });
  }
});
...
function loadThreadData() {
  ...
  onValue(threadRef, async (snapshot) => {
    ...
    threadAuthorUid = thread.authorUid; // Track for "Author" badge
...
async function createPostElement(post, postId, isOP, allPosts = {}) {
  ...
  let badgesHtml = renderBadges(meta);
  
  // Add "Author" badge if this is the thread creator
  if (post.authorUid === threadAuthorUid) {
    badgesHtml = `<span class="author-badge">Author</span>` + badgesHtml;
  }
      ...
      postCard.innerHTML = `
        <div class="post-sidebar" data-author-uid="${post.authorUid}">
          ...
          <div class="post-author-info">
            <span class="post-author-name" style="cursor: pointer;">${escapeHtml(meta.username || post.authorName)}</span>
