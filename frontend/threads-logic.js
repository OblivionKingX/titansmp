...
async function renderThreads(threads) {
  ...
  const threadPromises = threads.map(async (thread) => {
    ...
    let categoryHtml = '';
    if (thread.category) {
      categoryHtml = `<span class="badge-tag badge-${thread.category.toLowerCase()}">${thread.category.toUpperCase()}</span> `;
    }
    ...
    threadRow.innerHTML = `
      ...
      <div class="thread-info">
        <h3>${categoryHtml}${escapeHtml(thread.title)} ${statusHtml}</h3>
        <div class="thread-meta" style="display: flex; align-items: center; gap: 8px; flex-wrap: wrap;">
          Started by <span style="display: flex; align-items: center; gap: 6px;">${escapeHtml(meta?.username || thread.authorName)} ${badgesHtml}</span>, ${getTimeAgo(thread.timestamp)}
        </div>
      </div>
      ...
      <div class="thread-last-post">
        ...
        <div class="last-post-author" style="font-size: 0.75rem; color: var(--text-secondary);">Last reply by ${escapeHtml(thread.lastAuthorName || thread.authorName)}</div>
      </div>
    `;
    return threadRow;
  });
